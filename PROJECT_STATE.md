...
- [x] CHAT-FIX-GROQ-KEY-1: hardened Groq key handling (committed to **main**) — a CRLF-terminated Vault secret had produced `httpx.LocalProtocolError` → opaque **500** at `/api/v1/chat`, and the raw key had leaked into `chat.unexpected_error` logs. Fixes: `_load_groq_api_key()` + `GroqClient.__init__` now `.strip()` the key; `GroqClient.chat` catches `LocalProtocolError` → `GroqUnavailableError` (clean 503, static message, no key echo); `redact()` gains a `gsk_…` pattern + a prefix-less `Bearer/Basic/Token` pattern. Commits **d6f686a** (fixes) + **c12bd81** (RUNBOOK note: dev Vault is `server -dev`/ephemeral, so `secret/llm/groq_api_key` must be reseeded after any vault-container recreate). **Verified live 2026-06-28**: chat POST → 200 with answer + `trace_id`, `model=llama-3.3-70b-versatile`, log event `chat.complete`, zero key-pattern hits in logs.
- [x] CHAT-FIX-TITLE-ONLY-1: title-only issues now classify without error (committed **main 94d0fd5**). Root cause (confirmed via the captured Groq 400 body, *not* the empty-content hypothesis): the model emitted an `extract_entities` tool call that wasn't in `request.tools` for the classify-only page — the static system prompt unconditionally advertises every tool — so Groq rejected the turn with `tool_use_failed` (HTTP 400); an empty body just made the model reach for the extra tool. Fixes: (1) `classify_issue` `body` made optional (`required: ["title"]`) so a title-only issue isn't handed a hallucinated body; (2) orchestrator advertises the **full** `TOOL_DEFINITIONS` to Groq (so it never rejects a known-tool call) and appends a runtime "you may call ONLY: <enabled>" scope line to the system prompt, with `enabled_tools` still enforced at dispatch; (3) `GroqClient` logs the real provider status+body server-side (redacted, `groq.http_status_error`) and raises a clean **"The chat service is temporarily unavailable. Please try again."** instead of raw `Groq HTTP error 400`. **Verified live 2026-06-29**: title-only classify → 200 (label=question 0.346, trace_id); forced Groq error → clean UI message + real reason logged server-side; 35/35 chat unit+integration tests pass.
...
# CHAT-FIX-GROQ-KEY-1 groq key hardening + live verify (2026-06-28)
uv run ruff check app
docker compose up -d --build api
# reseed Groq key (trim-safe; value never in argv/history):
#   $key = ((Get-Content .env | ? { $_ -match '^GROQ_API_KEY=' }) -replace '^GROQ_API_KEY=','').Trim()
#   $key | docker compose exec -T vault sh -c 'k=$(tr -d "\r\n"); printf "%s" "$k" | VAULT_ADDR=http://127.0.0.1:8200 VAULT_TOKEN=dev-root-token vault kv put secret/llm groq_api_key=-'
# verify: POST /api/v1/chat -> 200 with answer + trace_id; log event chat.complete; zero key-pattern hits
```
